  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 503) {
        return res.status(503).json({ message: "Model is loading, please retry later" });
      }
      return res.status(response.status).json({ message: errorText || "Error from HF API" });
    }

    if (contentType && contentType.includes("application/json")) {
      const jsonData = await response.json();
      if (jsonData.error) {
        return res.status(503).json({ message: jsonData.error });
      }
      return res.status(200).json({ photo: null, message: "Model is loading, retry soon" });
    }
